#define LS_STATE_DIR_FIRST	0
#define LS_STATE_WAIT_FIRST	1
#define LS_STATE_ENERGY_FIRST	2
#define LS_STATE_DIR_SECOND	3
#define LS_STATE_WAIT_SECOND	4
#define LS_STATE_ENERGY_SECOND	5
#define LS_STATE_CHECK_RHO	6
			       ...
			       uint*                     evals_performed)
{
	...
	char ls_mode = LS_STATE_DIR_FIRST;
	...
	while ((iteration_cnt < DockConst->max_num_of_iters) && (rho > DockConst->rho_lower_bound))
	{
		switch (ls_mode) {
			...
			case LS_STATE_DIR_SECOND:
				...
				ls_mode = LS_STATE_WAIT_SECOND;
				write_to_channel_genotype = true;
				read_from_channel_energy = false;
			break;
			// ---------------------------------------------------------------------------------------------------------
			case LS_STATE_WAIT_SECOND:
				ls_mode = LS_STATE_ENERGY_SECOND;
				write_to_channel_genotype = false;
				read_from_channel_energy = true;
			break;
			// ---------------------------------------------------------------------------------------------------------
			...
			case LS_STATE_CHECK_RHO:
				...
				ls_mode = LS_STATE_DIR_FIRST;
				write_to_channel_genotype = false;
				read_from_channel_energy = false;
			break;
			// ---------------------------------------------------------------------------------------------------------
		} // End of switch (ls_mode)
