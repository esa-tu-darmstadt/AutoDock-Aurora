...
channel float chan_Intere2Manager_intere;
channel float chan_Intrae2Manager_intrae;
...

{
	...
	{	
		...
		write_channel_altera(chan_GA2Manager_cnt,  pop_cnt);
		// --------------------------------------------------------------
	...
	while ((eval_cnt < DockConst->num_of_energy_evals) && (generation_cnt < DockConst->num_of_generations))
	{
		...
		GlobPopulationNext[0] = GlobPopulationCurrent[best_entity_id];
		...
		for (new_pop_cnt = 1; new_pop_cnt < DockConst->pop_size; new_pop_cnt++)
		{
			...
			mode = 2;
			write_channel_altera(chan_GA2Manager_mode, mode);
			write_channel_altera(chan_GA2Manager_cnt,  new_pop_cnt);
			...
			for (uint gene_cnt = 0; gene_cnt < ACTUAL_GENOTYPE_LENGTH; gene_cnt++) {
				GlobPopulationCurrent[new_pop_cnt*ACTUAL_GENOTYPE_LENGTH + gene_cnt] = offspring_genotype[gene_cnt];
			}
