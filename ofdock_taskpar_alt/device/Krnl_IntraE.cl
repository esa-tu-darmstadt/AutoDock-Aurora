...
while(1) {
	...
	{	
		...
		if ((distance_leo < 8.0f) && (distance_leo < 20.48f)) 
		{
			...
			intraE+= ((KerConst->dspars_S_const[atom1_typeid] + 
				 DockConst->qasp*fabs(KerConst->atom_charges_const[atom1_id]))*KerConst->dspars_V_const[atom2_typeid] + 
				 (KerConst->dspars_S_const[atom2_typeid] + 							   
				 DockConst->qasp*fabs(KerConst->atom_charges_const[atom2_id]))*KerConst->dspars_V_const[atom1_typeid]) * DockConst->coeff_desolv*exp(-distance_leo*distance_leo/25.92f);
	...
	// --------------------------------------------------------------
	write_channel_altera(chan_Intrae2Manager_intrae, intraE);
	// --------------------------------------------------------------
