...
while(valid) {
	...
	while( (valid_active != PIPE_STATUS_SUCCESS) && (valid_energy != PIPE_STATUS_SUCCESS)) {
		valid_active = read_pipe(pipe00ga2ls00off900active, &active);
		valid_energy = read_pipe(pipe00ga2ls00ls900energy,  &current_energy);
	}
	...
	if (valid) {
		...
		for (uchar i=0; i<DockConst_num_of_genes; i++) {
			read_pipe_block(pipe00ga2ls00ls900genotype, &genotype [i]);
		}
		...
		while ((iteration_cnt < DockConst_max_num_of_iters) && (rho > DockConst_rho_lower_bound)) {	
			...
			write_pipe_block(pipe00ls2arbiter00ls900end, &tmp_int);
/*
...
*/
			...
			for (uchar i=0; i<DockConst_num_of_genes; i++) {
				float tmp_prng;
				read_pipe_block(pipe00prng2ls900float00prng, &tmp_prng);
/*
				mem_fence(CLK_CHANNEL_MEM_FENCE);
*/
				...
				write_pipe_block(pipe00ls2conf00ls900genotype, &tmp3);
			...
			while( (intra_valid != PIPE_STATUS_SUCCESS) || (inter_valid != PIPE_STATUS_SUCCESS)) {
				...
				if (intra_valid != PIPE_STATUS_SUCCESS) {
					intra_valid = read_pipe(pipe00intrae2storels00ls900intrae, &energyIA_LS_rx);
				}
				else if (inter_valid != PIPE_STATUS_SUCCESS) {
					inter_valid = read_pipe(pipe00intere2storels00ls900intere, &energyIE_LS_rx);
				}
			}
		...
		for (uchar i=0; i<DockConst_num_of_genes; i++) {
			if (i == 0) {
				...
				write_pipe_block(pipe00ls2ga00ls900evalenergy, &evalenergy);	
			}
/*
			mem_fence(CLK_CHANNEL_MEM_FENCE);
*/

			write_pipe_block(pipe00ls2ga00ls900genotype, &genotype [i]);
		}
