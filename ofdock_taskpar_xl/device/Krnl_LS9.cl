...
{
	#if 0
	printf("\nLS9: DockConst_max_num_of_iters: %u\n",   DockConst_max_num_of_iters);
	printf("LS9: DockConst_rho_lower_bound: %f\n",      DockConst_rho_lower_bound);
	printf("LS9: DockConst_base_dmov_mul_sqrt3: %f\n",  DockConst_base_dmov_mul_sqrt3);
	printf("LS9: DockConst_num_of_genes: %u\n",  	    DockConst_num_of_genes);
	printf("LS9: DockConst_base_dang_mul_sqrt3: %f\n",  DockConst_base_dang_mul_sqrt3);
...
*/
		valid_active = read_pipe(chan_GA2LS_Off9_active, &active);
		valid_energy = read_pipe(chan_GA2LS_LS9_energy,  &current_energy);
	}
/*
...
*/
	...
	if (valid) {
		...
		LOOP_FOR_LS9_READ_INPUT_GENOTYPE:
...
*/
			float tmp_gene;
			read_pipe_block(chan_GA2LS_LS9_genotype, &tmp_gene);
...
*/
			read_pipe_block(chan_GA2LS_LS9_genotype, &genotype [i]);
			#endif
		}
...
/*
			...
			write_pipe_block(chan_LS2Arbiter_LS9_end, &tmp_bool);
*/
			int tmp_int =  (fixpt_rho < DockConst_rho_lower_bound)?0:1;
			write_pipe_block(chan_LS2Arbiter_LS9_end, &tmp_int);
			#else
/*
...
*/
				write_pipe_block(chan_LS2Conf_LS9_genotype, &tmp3);
				#endif
...
*/
					intra_valid = read_pipe(chan_Intrae2StoreLS_LS9_intrae, &energyIA_LS_rx);
				}
/*
...
*/
					inter_valid = read_pipe(chan_Intere2StoreLS_LS9_intere, &energyIE_LS_rx);
				}
			}
...
*/
				write_pipe_block(chan_LS2GA_LS9_evalenergy, &evalenergy);	
			}
/*
			mem_fence(CLK_CHANNEL_MEM_FENCE);
*/
...
*/
			...
			write_pipe_block(chan_LS2GA_LS9_genotype, &tmp_float);
			#else
/*
			write_channel_altera(chan_LS2GA_LS9_genotype, genotype [i]);
*/
			write_pipe_block(chan_LS2GA_LS9_genotype, &genotype [i]);
			#endif
		}
